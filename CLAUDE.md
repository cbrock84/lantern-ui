...
4. After the answer, add or update the entry in `decisions.md` in the same
   change set as the work it affects, using the entry format defined at the top
   of that file: question, options, choice, date, decided by, status (Open,
   Decided or Done only), progress and links.
5. A reasonable default Claude picks without asking is still logged, with
   "Decided by: Claude (default)", so the owner can revisit it.
